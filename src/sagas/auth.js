...
import { logoutUser, loginUser,SignUser } from '../api/auth';
...
export function* fetchSingUp({ payload: { email, password } }) {
  try {
    yield call(SignUser, email, password);
    yield call(fetchUser);
    yield put(push('/wallets'));
  } catch (e) {
    yield put(actions.failLogin('Oups! Error occurs, please try again later.'));
...
export function* fetchSignUpSaga() {
  yield takeEvery(types.FETCH_SIGNUP, fetchSingUp);
}
